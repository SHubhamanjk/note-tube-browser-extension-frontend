...
class MedhaAPI {
  constructor() {
    this.baseURL = 'https://note-tube-be-prod-849608379958.asia-south1.run.app';
  ...
  async downloadNotesPdf(tutorialId) {
    return this.request(`/notes/tutorial/${tutorialId}/download`, {
      method: 'GET'
    });
  }
  ...
  async generateQuiz(tutorialId, fromTimestamp = null, toTimestamp = null) {
    let url = `/quizzes/tutorial/${tutorialId}/generate`;
    const params = new URLSearchParams();
    if (fromTimestamp) params.append('from_timestamp', fromTimestamp);
    if (toTimestamp) params.append('to_timestamp', toTimestamp);
